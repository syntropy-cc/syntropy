...
EOF
    ...
    log_debug "Generated ${LAYER_NAME}_summary.md at: $output_file"
    return 0
}
...
copy_detailed_log() {
    ...
    if [[ -f "$log_file" ]]; then
        ...
        log_debug "Copied ${LAYER_NAME}_detailed.log to: $output_file"
        return 0
    else
...
EOF
    ...
    for layer_dir in "$report_dir"/*/; do
        if [[ -d "$layer_dir" ]]; then
            ...
            local summary_file="$layer_dir/${layer_name}_summary.md"
            
                    if [[ -f "$summary_file" ]]; then
            echo "- [$layer_name]($layer_name/${layer_name}_summary.md)" >> "$consolidated_file"
        fi
        fi
    done
